...
class yuri(commands.Cog):
    def __init__(self, bot):
        ...
        self.nsfw_yuri_endpoint = "/nsfw/yuri"  # NSFW yuri endpoint


    def fetch_nsfw_yuri_image(self):
        try:
            response = requests.get(f'{self.base_url}{self.nsfw_yuri_endpoint}')
    ...
    @app_commands.command(name="nsfw_yuri", description="Fetches an NSFW yuri image")
    @app_commands.guilds(discord.Object(id=guild_id))
    async def nsfw_yuri(self, interaction: discord.Integration):
        image_url = self.fetch_nsfw_yuri_image()
        if interaction.channel and interaction.channel.nsfw:
            if image_url:
                embed = discord.Embed(
                    title="NSFW yuri Image",
                    description="Here is your requested yuri image!",
