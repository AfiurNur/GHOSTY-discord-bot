...
class cum(commands.Cog):
    def __init__(self, bot):
        ...
        self.nsfw_cum_endpoint = "/nsfw/cum"  # NSFW cum endpoint


    def fetch_nsfw_cum_image(self):
        try:
            response = requests.get(f'{self.base_url}{self.nsfw_cum_endpoint}')
    ...
    @app_commands.command(name="nsfw_cum", description="Fetches an NSFW cum image")
    @app_commands.guilds(discord.Object(id=guild_id))
    async def nsfw_cum(self, interaction: discord.Integration):
        image_url = self.fetch_nsfw_cum_image()

        if interaction.channel and interaction.channel.nsfw:

            if image_url:
                embed = discord.Embed(
                    title="NSFW Cum Image",
                    description="Here is your requested NSFW cum image!",
