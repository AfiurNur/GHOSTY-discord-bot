...
class panties(commands.Cog):
    def __init__(self, bot):
        self.bot = bot
        self.base_url = "https://api.lunardev.group"  # Base URL for the LunarDev API
        self.nsfw_panties_endpoint = "/nsfw/panties"  # NSFW panties endpoint


    def fetch_nsfw_panties_image(self):
        try:
            response = requests.get(f'{self.base_url}{self.nsfw_panties_endpoint}')
    ...
    @app_commands.command(name="nsfw_panties", description="Fetches an NSFW panties image")
    @app_commands.guilds(discord.Object(id=guild_id))
    async def nsfw_panties(self, interaction: discord.Integration):
        image_url = self.fetch_nsfw_panties_image()

        if interaction.channel and interaction.channel.nsfw:

            if image_url:
                embed = discord.Embed(
                    title="NSFW panties Image",
                    description="Here is your requested NSFW panties image!",
