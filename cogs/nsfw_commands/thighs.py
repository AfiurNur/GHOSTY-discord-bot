...
class thighs(commands.Cog):
    def __init__(self, bot):
        ...
        self.nsfw_thighs_endpoint = "/nsfw/thighs"  # NSFW thighs endpoint


    def fetch_nsfw_thighs_image(self):
        try:
            response = requests.get(f'{self.base_url}{self.nsfw_thighs_endpoint}')
    ...
    @app_commands.command(name="nsfw_thighs", description="Fetches an NSFW thighs image")
    @app_commands.guilds(discord.Object(id=guild_id))
    async def nsfw_thighs(self, interaction: discord.Integration):
        image_url = self.fetch_nsfw_thighs_image()

        if interaction.channel and interaction.channel.nsfw:

            if image_url:
                embed = discord.Embed(
                    title="NSFW thighs Image",
                    description="Here is your requested NSFW thighs image!",
