...
class gif(commands.Cog):
    def __init__(self, bot):
        ...
        self.nsfw_gif_endpoint = "/nsfw/gif"  # NSFW gif endpoint


    def fetch_nsfw_gif_image(self):
        try:
            response = requests.get(f'{self.base_url}{self.nsfw_gif_endpoint}')
    ...
    @app_commands.command(name="nsfw_gif", description="Fetches an NSFW gif image")
    @app_commands.guilds(discord.Object(id=guild_id))
    async def nsfw_gif(self, interaction: discord.Integration):
        image_url = self.fetch_nsfw_gif_image()

        if interaction.channel and interaction.channel.nsfw:

            if image_url:
                embed = discord.Embed(
                    title="NSFW gif Image",
                    description="Here is your requested NSFW gif image!",
