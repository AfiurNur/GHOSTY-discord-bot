...
class jpg(commands.Cog):
    def __init__(self, bot):
        self.bot = bot
        self.base_url = "https://api.lunardev.group"  # Base URL for the LunarDev API
        self.nsfw_jpg_endpoint = "/nsfw/jpg"  # NSFW jpg endpoint


    def fetch_nsfw_jpg_image(self):
        try:
            response = requests.get(f'{self.base_url}{self.nsfw_jpg_endpoint}')
    ...
    @app_commands.command(name="nsfw_jpg", description="Fetches an NSFW jpg image")
    @app_commands.guilds(discord.Object(id=guild_id))
    async def nsfw_jpg(self, interaction: discord.Integration):
        image_url = self.fetch_nsfw_jpg_image()

        if interaction.channel and interaction.channel.nsfw:

            if image_url:
                embed = discord.Embed(
                    title="NSFW jpg Image",
                    description="Here is your requested NSFW jpg image!",
